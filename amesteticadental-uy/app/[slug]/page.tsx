...
export default async function TreatmentPage({ params }: Props) {
  ...
  return <main><JsonLd data={faqSchema} /><SiteHeader /><section className="page-hero shell"><p className="eyebrow">{page.eyebrow}</p><h1>{page.title}<br /><em>con una mirada propia.</em></h1><p>{page.intro}</p></section><section className="detail-section"><div className="shell detail-grid"><div><p className="eyebrow">EL ENFOQUE AM</p><h2>Decidir bien<br /><em>antes de avanzar.</em></h2></div><div><p>{page.detail}</p><p>Conocé el portfolio clínico que expresa este estándar y recibí novedades sobre la futura agenda en Uruguay.</p><Link className="text-link" href="/casos-clinicos">Ver portfolio clínico AM</Link></div></div></section><section className="faq-section shell"><p className="eyebrow">PREGUNTAS FRECUENTES</p>{localFaqs.slice(0, 4).map((faq) => <details key={faq.question}><summary>{faq.question}</summary><p>{faq.answer}</p></details>)}</section><section className="lead-section"><div className="shell lead-grid"><div><p className="eyebrow">PRÓXIMA APERTURA</p><h2>Noticias de la sede<br /><em>en Carrasco.</em></h2><p>Dejá tus datos para recibir novedades verificadas sobre la apertura y la futura agenda.</p></div><LeadForm /></div></section></main>;
}
